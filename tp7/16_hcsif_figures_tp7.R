...
CCM_RDS <- {
  ...
  fs <- fs[!grepl("_raw\\.rds$", fs)]
  if (!length(fs)) stop("尚无 buf1000 CCM 结果，请先跑完 01_ccm_buf1000.R")
  tail(sort(fs), 1)
}
...
# 1. 读取 + 模式分类（与 15 脚本一致）
# ===========================================================================
MAXTP <- 7L        # tp=7 截断版本: 丢掉末步 tp=8
ccm <- readRDS(CCM_RDS) %>% as.data.frame() %>%
  filter(y_var == SIF_Y, x_var == VPD_X, tp <= MAXTP)
...
ggsave(file.path(OUT, sprintf("heatmap_station_tp_sign_tp%d.png", MAXTP)), p_heat,
       width = 9, height = 11, dpi = 300)
cat("-> heatmap_station_tp_sign_tp7.png\n")
...
if (!ok) {
  ...
  p_map <- p_map + ggnewscale::new_scale_color() +
    ...
    labs(title = "VPD-SIF causal response patterns and Koppen climate zones",
         subtitle = sprintf("SIF = HCSIF buf1000; tp = 0..%d; n = %d stations", max(ccm$tp), nrow(map_df)),
         x = "Longitude", y = "Latitude") + theme_map

  ggsave(file.path(OUT, sprintf("map_stype_koppen_hcsif_tp%d.png", MAXTP)), p_map, width = 14, height = 9, dpi = 300)
  cat("-> map_stype_koppen_hcsif_tp7.png\n")
}
