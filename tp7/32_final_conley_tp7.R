...
OUT <- "data_proc/output_hcsif_buf1000"; M <- 7L; CUT <- 200
...
d <- r[, .(grp, var, beta, se = se_conley200, n_obs)]
NI <- d[grp == "inhibit_first", n_obs[1]]; NP <- d[grp == "promote_first", n_obs[1]]
d[, `:=`(z = beta / se)]
d[, `:=`(p = 2 * pnorm(-abs(z)),
         ...
         grp_cn = factor(fifelse(grp == "inhibit_first",
                                 sprintf("抑制组：得分高 = 越早脱离抑制（n = %d）", NI),
                                 sprintf("促进组：得分高 = 促进维持越久（n = %d）", NP)),
                levels = c(sprintf("抑制组：得分高 = 越早脱离抑制（n = %d）", NI),
                           sprintf("促进组：得分高 = 促进维持越久（n = %d）", NP))))]
d[, sig := p < .05]
...
p <- ggplot(d, aes(beta, cn, colour = dimen)) +
  ...
  labs(title = "转变时间得分的影响因素",
       subtitle = paste0("严格分类，tp = 0–7（末步截断）；标准化回归系数与 95% 置信区间；",
...
ggsave(file.path(OUT, sprintf("fig_final_conley%d_tp%d.png", CUT, M)), p,
