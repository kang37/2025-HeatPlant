...
OUT <- "data_proc/output_hcsif_buf1000"; M <- 7L; NPERM <- 4000
set.seed(42)
...
LBL <- a[, .(lab = sprintf("%s（n = %d，R² = %.3f）",
                           fifelse(grp[1] == "inhibit_first", "抑制组", "促进组"),
                           n_obs[1], r2[1])), by = grp]
LI <- LBL[grp == "inhibit_first", lab]; LP <- LBL[grp == "promote_first", lab]
pl[, `:=`(grp_cn = factor(fifelse(grp == "inhibit_first", LI, LP),
            levels = c(LI, LP)),
...
p <- ggplot(pl, aes(pct, cn, fill = method)) +
  ...
  labs(title = "三种变量重要性度量的对照",
       subtitle = paste0("各度量在组内归一化为百分比；Shapley 值加总恰等于 R²；tp = 0–7（末步截断）\n",
...
ggsave(file.path(OUT, sprintf("fig_importance_tp%d.png", M)), p, width = 10.5, height = 8, dpi = 300)
log_msg("已输出 fig_importance_tp", M, ".png")
