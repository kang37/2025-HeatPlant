...
pA <- ggplot(a, aes(tp, med)) +
  ...
  scale_x_continuous(breaks = 0:7, sec.axis = sec_axis(~ . * 8, name = "滞后天数",
                                                       breaks = seq(0, 56, 8))) +
  labs(title = "A  因果强度随滞后单调衰减（tp ≤ 7）",
       subtitle = "线为中位数，带为四分位区间；n = 904 站（tp 截断至 7）",
...
ggsave(file.path(OUT, "fig_rho_vs_tp_tp7.png"), p, width = 7.2, height = 7.4, dpi = 300)
cat("已保存 fig_rho_vs_tp_tp7.png\n")
