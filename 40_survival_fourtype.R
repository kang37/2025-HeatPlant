...
pat <- cm[, .(co = list(mean_coef)), by = .(stat_id = meteo_stat)][sapply(co, length) == 9]
...
log_msg("完整 9 个滞后的站 ", nrow(pat), " -> 四类站 ", pat[ty != "X 多次变号", .N],
...
fwrite(S[, .(stat_id, ty, start_dir, time, event, koppen_group)],
       file.path(OUT, "surv4_sample.csv"))
...
fwrite(zt, file.path(OUT, "surv4_phtest.csv"))
...
log_msg("非信息删失: 删失 = 观测窗口到 tp8, 由设计决定, 假定成立。")

# ===========================================================================
hdr("2. Kaplan-Meier 与 log-rank (n = 385)")
...
print(kt); fwrite(kt, file.path(OUT, "surv4_km.csv"))
log_msg("\n翻转时间四分位(步, 每步 8 天):")
...
# ===========================================================================
hdr("3. Cox 比例风险模型 (n = 385, 18 协变量, Conley 200km)")
...
fwrite(rbindlist(res), file.path(OUT, "surv4_cox_coef.csv"))

log_msg("\n-- 3.1 与离散时间模型交叉核对(同一 385 站) --")
...
fwrite(sens, file.path(OUT, "surv4_varset_sens.csv"))
...
fwrite(het, file.path(OUT, "surv4_group_interaction.csv"))
...
pb <- ggplot(fp, aes(HR, lab, colour = tag, shape = sig)) +
  ...
  labs(title = "(b) Cox 风险比", subtitle = "每 1 个标准差; 区间为 Conley 200km; HR>1 = 翻转更早",
...
ggsave(file.path(OUT, "fig_survival_fourtype.png"),
       (pa | pb) / pc + plot_layout(heights = c(1, 1.15)), width = 12, height = 11, dpi = 300)
log_msg("\n已写出 fig_survival_fourtype.png")
