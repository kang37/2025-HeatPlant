...
#   抑制组(始终抑制 + 抑制后促进)
#       始终抑制            -> 1
#       在 tp = k 处翻转    -> (M + 2) - k     k=M 得 2, k=1 得 M+1
#       数值越大 = 越早脱离抑制
#       始终抑制单独占 1, 不与"末步翻转"合并——两者一个是右删失、
#       一个是已观测事件, 合并会抹掉这个区别。
#   促进组(始终促进 + 先促进后抑制)
...
#
# 两组均为 1..M+1 共 M+1 档, 含义统一为"得分越高 = 停留在促进状态越久"。
# 相对生存模型仍有损失: 得分是有序类别而非风险过程, 不区分"何时被观测到"。
#
...
pat[, score := fifelse(
  ...
  fifelse(event == 1, (M + 2) - event_tp, 1),        # 抑制组
