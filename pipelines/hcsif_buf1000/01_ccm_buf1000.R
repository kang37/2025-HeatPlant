...
STAT_DIR <- file.path(PROJ, "data_raw/hcsif/station_v2")   # v2 提取，含 SIF_buf1000
METEO    <- file.path(PROJ, "data_raw/meteo_data_1961-2023")
OUT_DIR  <- file.path(PROJ, "data_proc/ccm_hcsif_buf1000")
...
for (v in c("SIF_buf1000", "vpd_mean")) {
...
grid <- CJ(y_col = c("SIF_buf1000_dt"),       # 仅 1000m 缓冲区 SIF
...
safe_io(saveRDS(res, file.path(OUT_DIR, paste0("ccm_hcsif_buf1000_vpd_", stamp, "_raw.rds"))),
        "save raw rds")
...
safe_io(saveRDS(res, file.path(OUT_DIR, paste0("ccm_hcsif_buf1000_vpd_", stamp, ".rds"))), "save rds")
safe_io(fwrite(res,  file.path(OUT_DIR, paste0("ccm_hcsif_buf1000_vpd_", stamp, ".csv"))), "save csv")
log_msg("写出: ", OUT_DIR, "/ccm_hcsif_buf1000_vpd_", stamp, ".{rds,csv}  (", nrow(res), " 行)")
