...
BUF_R    <- as.integer(Sys.getenv("HCSIF_BUF_R", "2000"))
...
# 替代方法：按年整块置换驱动 VPD(保留季节结构、破坏年际因果耦合)
...
run_ccm <- function(sid, y_col, x_col, tp_x) {
  ...
  # 固定种子：仅依赖 sid 与 tp → 三套缓冲区用相同随机流，逐站可复现
  ...
  tryCatch({
    ...
    # --- 替代数据显著性：按年整块置换驱动，在最大文库上重算 rho 建零分布 -----
    # 单文库(lib_max)、确定性交叉映射，观测与替代用同一估计量以保证 p 值可比。
    # 用与主 CCM 一致的估计量：最大文库、随机抽取全部有效向量(random=TRUE)。
    # random=FALSE 会取"前 lib_max 个"连续点(早期年份)，对含空档序列不具代表性，
    # 会把观测 rho 压到近 0，导致 p 值失真。
    xmap_lmax <- function(heat_vec) {
      dfa <- data.frame(time = seq_len(n), sif = full$y, heat = heat_vec)
      cm2 <- tryCatch(rEDM::CCM(dataFrame = dfa, E = best_E, Tp = 0,
                       columns = "heat", target = "sif",
                       libSizes = paste(lib_max, lib_max, 1),
                       sample = 1, random = TRUE, showPlot = FALSE),
                     error = function(e) NULL)
      if (is.null(cm2)) return(NA_real_)
      mean(as.data.table(cm2)[["heat:sif"]], na.rm = TRUE)
    }
    p_surr <- NA_real_; rho_null95 <- NA_real_
    if (N_SURR > 0) {
    rho_obs_s <- xmap_lmax(full$x)
    ...
    if (length(yrs) >= 3 && is.finite(rho_obs_s)) {
      ...
      M[cbind(di, yi)] <- dd$x_lag                 # 驱动值按 (doy, year) 存表
      pos <- match(dd$idx8, full$idx8)             # dd 行 -> full 行
      rho_null <- rep(NA_real_, N_SURR)
      for (s in seq_len(N_SURR)) {
        src <- sample(length(yrs))                 # 年份整块置换(保季节、破年际耦合)
        heat_full <- rep(NA_real_, n)
        heat_full[pos] <- M[cbind(di, src[yi])]
        rho_null[s] <- xmap_lmax(heat_full)
      }
...
grid <- CJ(y_col = paste0(SIF_COL, "_dt"),    # 该缓冲区 SIF(去趋势)
           x_col = c("vpd_mean_dt"),          # 仅 VPD->SIF
...
safe_io(saveRDS(res, file.path(OUT_DIR, sprintf("ccm_hcsif_%s_vpd_%s_raw.rds", OUT_TAG, stamp))),
        "save raw rds")
...
safe_io(saveRDS(res, file.path(OUT_DIR, sprintf("ccm_hcsif_%s_vpd_%s.rds", OUT_TAG, stamp))), "save rds")
safe_io(fwrite(res,  file.path(OUT_DIR, sprintf("ccm_hcsif_%s_vpd_%s.csv", OUT_TAG, stamp))), "save csv")
log_msg("写出: ", OUT_DIR, "/ccm_hcsif_", OUT_TAG, "_vpd_", stamp, ".{rds,csv}  (", nrow(res), " 行)")
